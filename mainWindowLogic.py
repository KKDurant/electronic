...
class MainWindowLogic():
    def __init__(self) -> None:# 初始化
        ...
        # 初始化UI显示
        # self.ui.label_glue
        # self.imggreenbar= cv2.imread(ROOT /"greenbar.jpg")
        # self.imggreenbar = cv2.cvtColor(self.imggreenbar,cv2.COLOR_RGB2BGR)
        # self.imgredbar= cv2.imread(ROOT /"redbar.jpg")
        # self.imgredbar = cv2.cvtColor(self.imgredbar,cv2.COLOR_RGB2BGR)
...
if __name__ == '__main__':
    ...
    mainwindow = MainWindowLogic(mainui)
    camManager=mainwindow.camManager
    mainwindow.startup()
    t1 = Thread(target=mainwindow.listen, args=())
    t1.start()
    mainui.show()
    app.exec()